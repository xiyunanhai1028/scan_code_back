# 创建数据库
create database if not exists `sky_project`;
...
(
    ...
    `number`      int            NOT NULL       DEFAULT '1' COMMENT '数量',
    `amount`      decimal(10, 2) NOT NULL COMMENT '金额',
    `sit_num`     int            not null comment '桌位号',
    `create_time` datetime                      DEFAULT NULL COMMENT '创建时间',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  AUTO_INCREMENT = 9
...
(
    `id`          bigint not null auto_increment comment '主键',
    `status`      int                           DEFAULT '0' COMMENT '店铺营业状态 0:停业 1:营业中',
    `name`        varchar(45) collate utf8_bin  default null comment '店铺名称',
    `image`       varchar(255) COLLATE utf8_bin DEFAULT NULL COMMENT '图片',
    `address`     varchar(200) collate utf8_bin default null comment '店铺位置',
  ...
  AUTO_INCREMENT = 4
  DEFAULT CHARSET = utf8mb3
  COLLATE = utf8_bin COMMENT ='店铺信息表';
...
(
    ...
    `dish_ids`    varchar(100) collate utf8_bin DEFAULT NULL COMMENT '菜品ids',
    `setmeal_ids` varchar(100) collate utf8_bin DEFAULT NULL COMMENT '套餐ids',
    description   varchar(100) collate utf8_bin default null comment '用户描述',
    images        varchar(100) collate utf8_bin default null comment '上传图片地址',
    status        varchar(2) collate utf8_bin   default '1' comment '评论状态：1：满意，0：不满意',
    `create_time` datetime NOT NULL COMMENT '创建时间',
    primary key (`id`) using btree
) engine = InnoDB
  default charset = utf8
  collate = utf8_bin comment ='评论表';